...
void cUIParameter::InitParameters(cParameter* Parameter1, cParameter* Parameter2, cParameter* Parameter3){
    ...
    m_pFont1 = new DadGFX::cFont(FONT12);
    m_pFont2 = new DadGFX::cFont(FONT20);
    m_pFont3 = new DadGFX::cFont(FONT36);
    ...
    m_pParamDynMain->setFont(m_pFont3);
    ...
    m_pParamStatMain->setFont(m_pFont2);
}
...
DadGFX::cFont*      cUISaveRestore::m_pFont1=nullptr;
DadGFX::cFont*      cUISaveRestore::m_pFont2=nullptr;
...
void cUISaveRestore::InitSaveRestore(){
    ...
    if(m_pSaveStatMain==nullptr){
        ...
        m_pFont1 = new DadGFX::cFont(FONT20);
        m_pFont2 = new DadGFX::cFont(FONT36);
...
void cUISave::Activate(){
    ...
    m_pSaveStatMain->setFont(m_pFont2);
...
//***********************************************************************************
// Display layers
DadGFX::cLayer* cUIMainParameter::m_pStaticLayer=nullptr;   // Static layer for persistent elements
DadGFX::cLayer* cUIMainParameter::m_pDynamicLayer=nullptr;  // Dynamic layer for real-time updates
    
// Font resources
DadGFX::cFont* cUIMainParameter::m_pFont=nullptr; // Font used for parameter names
...
void cUIMainParameter::InitMainParameter(){
    if(m_pStaticLayer == nullptr){

        // Load font resource for text rendering
        m_pFont = new DadGFX::cFont(FONT09);
        ...
        m_pStaticLayer->setFont(m_pFont);
    }
}
...
void cUIMainParameter::Activate(){
    ...
    if(m_pTabChilds.size() != 0){
        ...
        m_pDynamicLayer->drawRect(m_SelectedParameter*MENU_ITEM_WitDH, 0, MENU_ITEM_WitDH, MENU_HEIGHT, 2, MENU_SELETED_ITEM_COLOR);
        m_pDynamicLayer->drawFillRect(m_ActiveParameter*MENU_ITEM_WitDH, 0, MENU_ITEM_WitDH, MENU_HEIGHT, MENU_ACTIVE_ITEM_COLOR);
        m_pTabChilds[m_ActiveParameter].m_pChild->Activate();
    }
}
...
void cUIMainParameter::DeActivate(){
    m_pStaticLayer->changeZOrder(0);
    m_pDynamicLayer->changeZOrder(0);
    if(m_ActiveParameter >= 0){
        m_pTabChilds[m_ActiveParameter].m_pChild->DeActivate();
    }
}
...
void cUIMainParameter::Proceed(){
    if(m_ActiveParameter >= 0){
        // Handle encoder rotation for parameter selection
        if(m_Encoder0Increment != 0){
            m_SelectedParameter += m_Encoder0Increment;
            // Wrap selection around bounds
            if(m_SelectedParameter < 0){
                m_SelectedParameter = static_cast<int8_t>(m_pTabChilds.size())-1;
            } else if (m_SelectedParameter >= static_cast<int8_t>(m_pTabChilds.size())){
                m_SelectedParameter = 0;                        
            }
        ...
        if(m_Encoder0.getSwitchState() && (m_ActiveParameter != m_SelectedParameter)){
            if(m_pTabChilds[m_SelectedParameter].m_pChild->isMain() == false){
                m_pTabChilds[m_ActiveParameter].m_pChild->DeActivate();
                m_ActiveParameter = m_SelectedParameter;
                m_pTabChilds[m_ActiveParameter].m_pChild->Activate();
            }else{
                m_pTabChilds[m_ActiveParameter].m_pChild->DeActivate();
                DeActivate();
                m_pActiveUI = m_pTabChilds[m_SelectedParameter].m_pChild;
                m_pTabChilds[m_SelectedParameter].m_pChild->Activate();
            }
        }
        ...
        m_pTabChilds[m_ActiveParameter].m_pChild->Proceed();
    }
}
